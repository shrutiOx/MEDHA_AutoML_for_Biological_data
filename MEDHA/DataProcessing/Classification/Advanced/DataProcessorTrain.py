...
class DataPreprocessTrain():
    def __init__(self, 
        ...
        self.concatslicestart         = concatslicestart
        self.concatsliceend           = concatsliceend
    ...
    def GetDataConcat(self):
        ...
        data_2 =  df[df.columns[self.concatslicestart:self.concatsliceend]].values
        ...
        if str.lower(self.seqtype) == 'proteinpadded':
            ...
            print(' Using custom encoding scheme in train ')
        ...
        condata =torch.tensor( data_2).float()
        ...
        '''all data'''
        data   = data.to(device)
        condata   = condata.to(device)
        
        labels = labels.to(device)
        
        '''now this is the whole dataset. For training (specially K-FOLD) we will put this whole into the model'''
        whole_Dataset = TensorDataset(data,labels)

        
        '''now creating dataloaders'''
        whole_loader = DataLoader(whole_Dataset,batch_size=whole_Dataset.tensors[0].shape[0])
        sampledata   = data[0]

        
        return whole_Dataset,whole_loader,sampledata,data,labels,condata
