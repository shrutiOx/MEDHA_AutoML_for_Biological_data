...
class DartTrainer():
  ...
  def DARTTrain(   self,modelspace,
        ...
        def dartaccuracy(yHat, y):
            ...
            if (str.lower(lossfun) == 'div'):
                yget=(np.exp(yHat.detach().cpu()))
                yget = yget.to(device)
                accuracy = torch.mean(kl_divergence(y,yget))

                accuracy = torch.mean(accuracy)

                res = dict()

                res["divergence loss_acc"] = accuracy

                trainval_accuracydarts = res
                
                fromhere = accuracy.cpu()
                self.saveaccuracy.append(fromhere)
                
            elif (str.lower(lossfun) == 'mse'):
                lossfunc = nn.MSELoss()
                loss = lossfunc(yHat,y)
                accuracy = lossfunc(yHat,y)
                res = dict()
                res["mean squared loss_acc"] = loss
                trainval_accuracydarts = res        
                
                fromhere = accuracy.detach().cpu()
                self.saveaccuracy.append(fromhere)
        ...
        if 'mse' in str.lower(lossfun):
            criterion = nn.MSELoss() 
                            
        elif 'div' in str.lower(lossfun):
