...
class SemiManualDart_train(nn.Module):
    ...
    def KFoldCrossValidator(self,
        ...
        for fold, (train_idx,val_idx) in enumerate(splits.split(np.arange(len(crossvalidator_dataset)))):
            ...
            lossfun,optimizer = trainparter.OptandLoss(model,learning_rate,momentum,L2lambda,'Adam',lossfuntype)
            trainAcc,skAccScore,losses,theBestModel,valloss = trainparter.trainTheModel(model,num_epochs,lossfun,optimizer,train_loader_Kfold,test_loader_Kfold,trainAcc,SkAcc,losses,valloss,predtype)

            
            print("Training Loss,  Training Acc , Test Acc Validation loss of last epoch of this fold ", losses[-1],trainAcc[-1],skAccScore[-1],valloss[-1])
            history['train_loss'].append(losses)
            history['train_acc'].append(trainAcc)
            history['validation_acc'].append(skAccScore)
        ...
        avg_val_acc = np.mean(history['validation_acc'])
        avg_val_loss = np.mean(history['validation_loss'])


        print('Performance of {} fold cross validation'.format(k))
        print("Average Training Loss, Average Training Acc, Average validation Acc, Average validation loss ",avg_train_loss,avg_train_acc,avg_val_acc,avg_val_loss) 
        
        return history['train_loss'],history['train_acc'],history['validation_acc'],history['validation_loss'], avg_train_loss,avg_train_acc,avg_val_acc,avg_val_loss,model
